...
def main():
    if (len(sys.argv) != 3):
        raise Exception(
            "Usage: python confparser.py [Next Config] [Current Config]")
    ...
    indexserver_sql_knobs = {'sql_executors',
                             'max_sql_executors', 'plan_cache_size'}
    indexserver_execution_knobs = {
        'max_concurrency', 'default_statement_concurrency_limit', 'num_cores'}
